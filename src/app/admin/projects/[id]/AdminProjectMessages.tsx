...
export default function AdminProjectMessages({ bookingId }: { bookingId: string }) {
  ...
  return (
    <div className="p-5 bg-white border border-ink-900/15 ">
      <h2 className="panel-title mb-4">Customer Communication</h2>

      <div className="min-h-[180px] max-h-72 overflow-y-auto space-y-2 mb-4 bg-stone-50 p-3">
        {loading ? (
          ...
          messages.map(m => (
            <div key={m.id} className={`flex ${m.sender === 'homeserve' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[75%] px-3 py-2 text-xs ${
                ...
                {m.body && <p>{m.body}</p>}
                <p className={`text-[0.6875rem] mt-1 ${m.sender === 'homeserve' ? 'text-white/90' : 'text-stone-400'}`}>
                  {m.sender === 'homeserve' ? 'HomeServe' : 'Customer'} ·{' '}
