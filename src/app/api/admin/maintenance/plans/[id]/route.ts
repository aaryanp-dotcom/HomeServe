...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  ...
  const { data: before } = await a.admin.from('maintenance_plans').select('name, annual_price, is_active, discount_percent, included_visits, service_credit_amount').eq('id', params.id).maybeSingle()
  ...
  await logAdmin(a.admin, a.user.id, {
    action: 'plan.update', entity_type: 'maintenance_plan', entity_id: params.id,
