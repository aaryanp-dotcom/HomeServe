...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: before } = await a.admin.from('maintenance_services').select('indicative_price_from, indicative_price_to, is_active').eq('id', id).maybeSingle()
  ...
  await logAdmin(a.admin, a.user.id, {
    action: 'service.update', entity_type: 'maintenance_service', entity_id: id, summary: `Edited service ${v.name}`,
